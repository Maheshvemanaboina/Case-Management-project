({
    ...
    setContentCategories : function(component, helper, contentCategories){
		...
		contentCategories = [{ label: "--None--", value: "" },...contentCats];
   ...
   toggleContentCategoriesMessage : function(component, isContentDisabled) {
    component.set("v.isContentDisabled", isContentDisabled);

    if (isContentDisabled) {
        $A.util.addClass(component.find('relatedArticleInfo'), "slds-hide");
        $A.util.removeClass(component.find('relatedArticleInfoDisabled'), "slds-hide");
    } else {
        $A.util.removeClass(component.find('relatedArticleInfo'), "slds-hide");
        $A.util.addClass(component.find('relatedArticleInfoDisabled'), "slds-hide");
    }
},
    ...
    getApexParamsOnSaveNew : function(component, helper, cvId) {
        ...
        let params = ({
			...
			//B2B-326
            categoryObjArrayData       :JSON.stringify(component.get("v.contentManagentCategryList")),
            /*B2B-326*/
			contentType : 				component.get("v.contentTypeSelected")
        });
    ...
    setEditForm : function(component, helper, selectedRecordId){
        ...
        .then($A.getCallback(function(result) {
           ...
           //326 
            ...
            // Extract the names from existingH5Accounts
            //let existingH5AccountsNames = existingH5Accounts.map(function(account) {
            //    return account.Name;
            //});
            // Extract the names from existingH1Accounts
            //let existingH1AccountsNames = existingH1Accounts.map(function(account) {
            //    return account.Name;
            //});
    ...
    getApexParamsOnSaveEdit : function(component, cvId, params){
        ...
        let saveParams = ({
			...
			//B24-326
            categoryObjArrayData       :JSON.stringify(component.get("v.contentCategoriesObjList")),
            //B2B-326
...
saveEditMethod : function(component,helper,cvId,apexMethodName,parameteronEdit){
    ...
    updateTileAccounts: function(component, helper, params) {
        ...
        var tileSelectedID = component.get("v.tileSelectedID");
        var accountsToDelete = [];
        var accountsToInsert = [];
        
        console.log('existingH5AccountsIds--->',existingH5AccountsIds);
	    console.log('existingH1AccountsIds--->',existingH1AccountsIds);
